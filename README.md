...
## Задание 2. Раздел Connected Dapps в Настройках.
...
**Описание.** В режиме расширения для Chrome кошёлк поддерживает протокол TON Connect. Это заметно, когда посещаешь dapp-сайты (например, getgems.io). Неоьходимо добавить в Настройках раздел управления подключёнными dapp-сайтами согласно [макетам](https://www.figma.com/file/4zlzG0ShKBrhxT6yUZlqtk/MyTonWallet-Design-Mobile-Public?node-id=3277%3A33896&t=vcntzMfbHADbRQrk-0).

## Задание 3. Экран Swap
...
**Описание.** На главном экране должна появиться кнопка Swap, при её нажатии происходит переход к [экрану Swap](https://www.figma.com/file/4zlzG0ShKBrhxT6yUZlqtk/MyTonWallet-Design-Mobile-Public?node-id=2632%3A48601&t=vcntzMfbHADbRQrk-0) (открытие модального окна/панели). Небходимо сделать mocked data для списка доступных токенов и их балансов. С учётом mocked-данных должны отображаться ошибки (по аналогии с экраном Send).
